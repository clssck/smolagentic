...
class ManagerAgent(BaseAgent):
    ...
    def get_system_prompt(self) -> str:
        ...
        return f"""You are an ultra-fast Manager Agent that routes queries to specialized agents.

Available Specialized Agents:
{agent_list}

Your job is to:
1. Quickly analyze the user query
2. Route to the most appropriate specialized agent
3. Handle simple queries directly if no specialized agent is needed

Routing Rules:
- Research/web search queries → research_agent
- Knowledge base/document queries → rag_agent  
- Code generation/programming → code_agent
- Image/vision queries → vision_agent
- Simple greetings/math/basic questions → handle directly
